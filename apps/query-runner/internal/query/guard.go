...
func ValidateSQL(sql string) error {
	...
	if trimmed == "" {
		return runner.NewStatusError(http.StatusBadRequest, "sql is required", nil)
	}
	if hasStackedStatements(trimmed) {
		return runner.NewStatusError(http.StatusBadRequest, "Only a single SQL statement is allowed", nil)
	}
	...
	case "select", "with", "show", "explain":
		return nil
	default:
		return runner.NewStatusError(http.StatusBadRequest, "Only read-only query statements are allowed", nil)
	}
}
