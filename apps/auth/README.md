...
| `POST /auth/agent-tokens/exchange` | Внутренний обмен agent JWT на короткий access JWT для `ir-api` |
...
| `INVESTIGATION_AGENT_EXCHANGE_CLIENT_ID` / `…_CLIENT_SECRET` | — | confidential client `ir-api` для backend-only exchange; обе переменные обязательны вместе, secret ≥32 bytes |
