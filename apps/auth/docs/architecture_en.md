...
**Investigation agent delegation** (`POST /auth/agent-tokens/investigation`). After normal JWT verification and the live-session guard, Auth forwards the original Bearer to `GET {PLATFORM_API_BASE_URL}/projects/{project_id}`. A successful project read allows Auth to issue a separate Ed25519 JWT with `typ=agent+jwt`, the single audience `ir-mcp`, fixed scopes, project/investigation binding, and a four-hour TTL. The token is not stored in the database, is never logged, and does not replace the user's `access+jwt` for Platform REST APIs.

**Investigation agent exchange** (`POST /auth/agent-tokens/exchange`). Only the configured confidential `ir-api` client can present an agent JWT and receive a normal access JWT with the same `sub`/`sid` and `ACCESS_TOKEN_TTL_SEC`. Auth rechecks the signature, `investigation.gateway.read`, and the original live session before issuance. IR uses this short token for the existing Gateway → Platform Secrets chain; it is never returned to the agent or persisted.
